...
class delta:   
    def __init__(self, alg = 'sha256', obj = None):
        self.alg = alg
        self.obj = obj
        return None
    
    #dictionary to verify hash algorithm is allowed
    ...
    def _return_hash(self,alg,obj=None):
        try:
    ...
    #identifies delta
    def result(self,oldfile, newfile, exportfile, block = 65536, statistics = False):
        if statistics == True:
        ...
        with open(oldfile) as file:
            for line in file:
                ...
                hasher_old_row = return_hash(line)
        ...
        with open(newfile) as file:
            for line in file: 
                line = line.strip().encode('utf-8') 
                hasher_new_row = return_hash(line)
